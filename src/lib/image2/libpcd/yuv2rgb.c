...
#ifndef _REENTRANT
#define SPEED_STATIC static     /* a bit faster, but not thread-save */
#endif
...
{
    SPEED_STATIC unsigned char  red[3072];
    SPEED_STATIC unsigned char  blue[3072];
    int                         bytes,maxx;

    switch (type) {
    case PCD_TYPE_GRAY:       bytes = 1; break;
    case PCD_TYPE_RGB:        bytes = 3; break;
    case PCD_TYPE_BGR:        bytes = 3; break;
    case PCD_TYPE_LUT_SHORT:  bytes = 2; break;
    case PCD_TYPE_LUT_LONG:   bytes = 4; break;
    default:
