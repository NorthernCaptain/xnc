...
static char *moduleid = 
        "@(#)$Id: fsmagic.c,v 1.2 2002/08/05 10:43:05 leo Exp $";
#endif        /* lint */
...
struct stat *sb;
{
...
#else
                (void) sprintf(tmp_chr, "character special (%d/%d)",
                        major(sb->st_rdev), minor(sb->st_rdev));
        ...
        case S_IFBLK:
                ...
                (void) printf("block special (%d/%d)",
                        major(sb->st_rdev), minor(sb->st_rdev));
#else
                (void) sprintf(tmp_chr, "block special (%d/%d)",
                        major(sb->st_rdev), minor(sb->st_rdev));
                strcat(magic_out, tmp_chr);
#endif
                return 1;
        ...
        case S_IFLNK:
                {
                        ...
                        if ((nch = readlink(fn, buf, BUFSIZ-1)) <= 0) {
                                ...
                                ckfprintf(stdout, "unreadable symlink (%s).", 
                                      strerror(errno));
#else
                                sprintf(tmp_chr, "unreadable symlink (%s).", 
                                      strerror(errno));
                                strcat(magic_out, tmp_chr);
#endif
                                return 1;
